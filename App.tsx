...
import { generateDecadeImage, generateFutureImage } from './services/geminiService';
...
function App() {
    ...
    const [futureImage, setFutureImage] = useState<GeneratedImage | null>(null);
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [isDownloading, setIsDownloading] = useState<boolean>(false);
    const [appState, setAppState] = useState<'idle' | 'image-uploaded' | 'age-selection' | 'generating' | 'generating-future' | 'results-shown' | 'future-results-shown'>('idle');
    ...
    const handleGenerateFutureClick = async () => {
        if (!uploadedImage || !targetAge) return;
        ...
        setAppState('generating-future');
        setFutureImage({ status: 'pending' });

        try {
            const resultUrl = await generateFutureImage(uploadedImage, age);
            setFutureImage({ status: 'done', url: resultUrl });
            setAppState('future-results-shown');
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : "An unknown error occurred.";
            setFutureImage({ status: 'error', error: errorMessage });
            setAppState('future-results-shown'); // Still show results view, which will display the error
            console.error(`Failed to generate future image for age ${age}:`, err);
        }
    };
    ...
    const handleDownloadFutureImage = () => {
        if (futureImage?.status === 'done' && futureImage.url) {
            const link = document.createElement('a');
            link.href = futureImage.url;
            link.download = `past-forward-age-${targetAge}.jpg`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        }
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">Past & Future</h1>
                    <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">A journey through time, powered by AI.</p>
                </div>
                ...
                {appState === 'image-uploaded' && uploadedImage && (
                    <motion.div className="flex flex-col items-center gap-6" initial={{opacity: 0}} animate={{opacity: 1}}>
                         <PolaroidCard 
                            ...
                            status="done"
                         />
                         <p className="font-permanent-marker text-neutral-300 mt-4 text-xl tracking-wide">Where do you want to go?</p>
                         <div className="flex items-center gap-4 mt-2">
                            <button onClick={handleGenerateClick} className={secondaryButtonClasses}>
                                Travel to the Past
                            </button>
                            <button onClick={() => setAppState('age-selection')} className={primaryButtonClasses}>
                                See the Future
                            </button>
                         </div>
                    </motion.div>
                ...
                {appState === 'age-selection' && uploadedImage && (
                    <motion.div 
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center gap-4 mt-4 w-full max-w-xs">
                            <label htmlFor="age-input" className="font-permanent-marker text-xl text-neutral-300">Enter a target age</label>
                            <input
                                id="age-input"
                                type="number"
                                value={targetAge}
                                ...
                                placeholder="e.g., 22"
                                className="w-full bg-white/10 text-white text-center font-sans text-lg p-3 rounded-sm border-2 border-white/50 focus:border-yellow-400 focus:ring-yellow-400 focus:outline-none transition-colors"
                                min="1"
                                max="120"
                            />
                            <button onClick={handleGenerateFutureClick} className={primaryButtonClasses} disabled={!targetAge}>
                                Generate Future Photo
                            </button>
                             <button onClick={() => setAppState('image-uploaded')} className="text-neutral-400 hover:text-white transition-colors text-sm mt-2">
                                &larr; Go Back
                            </button>
                        </div>
                    </motion.div>
                     ...
                     <>
                        {isMobile ? (
                            <div className="w-full max-w-sm flex-1 overflow-y-auto mt-4 space-y-8 p-4">
                                {DECADES.map((decade) => {
                                    // FIX: Extract image object to help with type inference in JSX.
                                    const image = generatedImages[decade];
                                    return (
                ...
                {(appState === 'generating-future' || appState === 'future-results-shown') && futureImage && (
                    <motion.div 
                    ...
                    >
                        <PolaroidCard
                            caption={appState === 'generating-future' ? `Traveling to age ${targetAge}...` : `You at age ${targetAge}`}
                            status={futureImage.status}
                            imageUrl={futureImage.url}
                            error={futureImage.error}
                            onDownload={handleDownloadFutureImage}
                            isMobile={isMobile}
                        />
                        <div className="h-20 mt-8 flex items-center justify-center">
                            {appState === 'future-results-shown' && (
                                <div className="flex flex-col sm:flex-row items-center gap-4">
                                    <button onClick={handleTryAnotherAge} className={primaryButtonClasses}>
                                        Try Another Age
                                    </button>
                                    <button onClick={handleReset} className={secondaryButtonClasses}>
                                        Start Over
